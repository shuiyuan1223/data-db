...
DATA_FIELDS_CATALOG: list[dict] = [
    ...
    # {"field": "device_type", "desc": "设备类型"},
    # {"field": "domain", "desc": "查询/兴趣领域"},
    # {"field": "occupation", "desc": "职业"},
    # {"field": "physiological_stage", "desc": "生理阶段"},
    # {"field": "personality_tags", "desc": "性格/行为标签"},
    # {"field": "preferred_sports", "desc": "偏好运动项目（运动领域专属）"},
    # {"field": "sport_mastery", "desc": "运动技能水平（运动领域专属）"},
    # {"field": "sport_goal", "desc": "运动目标（运动领域专属）"},
    # {"field": "core_health_concerns", "desc": "核心健康关注点（健康领域专属）"},
    ...
    #
    # # --- 人群基线与元数据 (Population & Metadata) ---
    # {"field": "percentile",               "desc": "人群分布百分位 (10/25/50/75/90)"},
    # {"field": "age_group",                "desc": "所属年龄段 (如 18-24)"},
...
# Placeholders:
#   $persona_block   — rendered persona card (see build_persona_block)
#   $catalog_topic   — single knowledge catalog item string
# ---------------------------------------------------------------------------
...
def build_messages(persona: dict, catalog_topic: str) -> list[dict]:
    ...
    fields_catalog = _render_fields_catalog(DATA_FIELDS_CATALOG)  # 加这行
    user_content = USER_PROMPT_TEMPLATE.substitute(
        persona_block=persona_block,
        catalog_topic=catalog_topic,
        persona_id=persona.get("id"),
        fields_catalog=fields_catalog,  # 加这个
    )
    return [
